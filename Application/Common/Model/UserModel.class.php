		...
		if($data){
			if(!empty($data["newapass"])){
				...
				if(empty($data["oldpass"])){
					return array('status' => 0,'msg'=>'请输入原密码');
			     }
			     if(empty($data["newapass"])){
			     	return array('status' => 0,'msg'=>'请输入新密码');
			     }
			     if($data["newapass"]!=$data["newapass2"]){
			     	return array('status' => 0,'msg'=>'两次输入新密码不一致');
			     }
				$res = M("user")->where("user_id=".$_SESSION["user_id"])->find();
				if (!$res['salt']) {
					if($res['pass']!=md5($data["oldpass"])){
					return array('status' => 0,'msg'=>'原密码错误');
					}
				}else{
				...
				if ($data['oldpass'] == $data['newapass']) {
					return array('status' => 0,'msg'=>'新密码不能与近期用过密码相同');
				}

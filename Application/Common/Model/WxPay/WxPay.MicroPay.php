	...
	public function pay($microPayInput)
	{
		...
		while($queryTimes > 0)
		{
			...
			if($succResult == 2){
				...
				$ret = array('state'=>1,'errormsg'=>'pay success','data'=>$queryResult);
				return $ret;
				return $queryResult;
			} else {//订单交易失败
				$ret = array('state'=>0,'errormsg'=>'failed in query');
				return $ret;
				return false;
			}
		}
		...
		{
			$ret = array('state'=>0,'errormsg'=>'cancel failed');
			return $ret;
			throw new WxpayException("撤销单失败！");
		}

		$ret = array('state'=>0,'errormsg'=>'failed in end');
		return $ret;
		return false;
	}
